...
function medicalStatusPills(report){const topics=[...new Set((report.cards||[]).map(c=>c.topic).filter(Boolean))]; return `<section class="grid medical-summary-grid">${card('Sources',report.source_count||0,'info','PubMed/literature records')}${card('Hypothesis cards',report.card_count||0,'good','Safety-gated cards')}${card('Topics',topics.length,'info',topics.map(medicalTopicLabel).join(', ')||'none')}${card('Mode',report.mode||'literature_only','warn','No clinical action')}</section>`;}
function medicalTopicLabel(value){return titleCase(String(value||'').replace(/^gi[-_]/,''));}
function listItems(items){return (items||[]).map(x=>`<li>${esc(x)}</li>`).join('');}
function renderCaseResearchMap(report){const map=report.case_research_map; if(!map)return ''; const branches=(map.ranked_branches||[]).map(b=>`<article class="branch-item"><div class="branch-head"><div><div class="branch-title">${esc(medicalTopicLabel(b.topic))}</div><div class="muted">Evidence ${esc(b.evidence_count||0)} - confidence ${esc(titleCase(b.confidence||'unknown'))}</div></div><span class="pill info">Rank ${esc(b.rank||'')}</span></div><div class="branch-why">${esc(b.why_it_matters||'')}</div><div class="branch-detail-grid"><div class="branch-detail"><h4>What would change confidence</h4><ul>${listItems(b.what_would_change_confidence)}</ul></div><div class="branch-detail"><h4>Clinician-safe questions</h4><ul>${listItems(b.clinician_safe_questions)}</ul></div></div></article>`).join(''); return `<section class="medical-map"><div class="banner info"><strong>How to use this map</strong><div>${esc(map.purpose||'Research map only. Not medical advice.')}</div></div><div class="medical-map-grid"><div class="medical-map-panel"><h3>Known facts in this scenario</h3><ul>${listItems(map.known_facts)}</ul></div><div class="medical-map-panel"><h3>Open uncertainties</h3><ul>${listItems(map.uncertain_claims)}</ul></div></div><div class="medical-map-panel"><h3>Ranked research branches</h3><div class="branch-list">${branches||'<div class="muted">No ranked branches available.</div>'}</div></div></section>`;}
function medicalCardHtml(c,reportPath){const evidence=(c.evidence_for||[]).slice(0,3).map(e=>`<div class="medical-evidence"><strong class="medical-evidence-title">${esc(e.title||e.source_id)}</strong><div class="muted medical-evidence-meta">${e.year?esc(e.year)+' - ':''}${e.url?`<a href="${esc(e.url)}" target="_blank" rel="noreferrer">PubMed</a>`:esc(e.source_id||'source')}</div><div class="medical-evidence-excerpt">${esc(e.excerpt||'')}</div></div>`).join(''); return `<article class="card medical-card"><div class="row medical-topic-row"><span class="pill info medical-topic">${esc(medicalTopicLabel(c.topic))}</span><span><span class="pill ${statusClass(c.review_status)}">${esc(titleCase(c.review_status||'needs_review'))}</span><span class="pill warn">Human review required</span></span></div><h3>${esc(c.hypothesis)}</h3><div class="medical-mechanism">${esc(c.mechanism||'')}</div><div class="row medical-tests">${(c.safe_next_tests||[]).map(t=>`<span class="pill good">${esc(titleCase(t))}</span>`).join('')}</div><div><h4>Evidence</h4><div class="medical-evidence-list">${evidence||'<div class="muted">No evidence snippets attached.</div>'}</div></div><div><h4>Limits</h4><ul class="medical-limits">${(c.evidence_limits||[]).map(x=>`<li>${esc(x)}</li>`).join('')}</ul></div><div class="banner warn medical-safety">${esc(c.safety_label||'Literature-only. Not medical advice.')}</div><div class="toolbar medical-actions"><button onclick="reviewMedicalCard('${esc(c.card_id)}','approved_for_research_planning','${esc(reportPath||'')}')">Approve for planning</button><button onclick="reviewMedicalCard('${esc(c.card_id)}','needs_more_evidence','${esc(reportPath||'')}')">Needs more evidence</button><button onclick="reviewMedicalCard('${esc(c.card_id)}','rejected','${esc(reportPath||'')}')">Reject</button></div></article>`;}
